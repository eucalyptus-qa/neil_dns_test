...
for( my $i = 0; $i < @ip_lst; $i++ ){
	...
	if( does_It_Have($stripped_roll, "CLC") ){
		print "$this_ip : Disabling DNS on this Cloud-Controller\n";
		...
		print("ssh -o StrictHostKeyChecking=no root\@$this_ip \"sed --in-place 's/DISABLE_DNS=\\\"N\"/DISABLE_DNS=\\\"Y\\\"/' $ENV{'EUCALYPTUS'}/etc/eucalyptus/eucalyptus.conf \"\n");
		system("ssh -o StrictHostKeyChecking=no root\@$this_ip \"sed --in-place 's/DISABLE_DNS=\\\"N\\\"/DISABLE_DNS=\\\"Y\\\"/' $ENV{'EUCALYPTUS'}/etc/eucalyptus/eucalyptus.conf \" ");
