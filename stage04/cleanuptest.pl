...
# clean up running instances
    chomp($instIds=`ec2-describe-instances | grep INST | awk '{print \$2}'`);
    $instIds=~s/\n/ /g;
    print "INSTIDS: $instIds\n";
    if ($instIds) {
	system("date");
	$cmd = "ec2-terminate-instances $instIds";
	$rc = system($cmd);
	if ($rc) {
	    print "ERROR: failed - '$cmd'\n";
	    $done++;
...
# clean up addrs
    ...
    $done = 0;
    $poll_count = 0;
    while ($done == 0 && $poll_count < 10 ){
    system("date");
    $cmd = "euca-describe-addresses | grep admin";
    open(RFH, "$cmd|");
    while(<RFH>) {
    chomp;
    my $line = $_;
    ...
    if (@ips > 0) {
    	...
    	print "Could not find addresses yet, looking again in 10 s";
    	sleep(10);
    	$poll_count++;
    ...
    for ($i=0; $i<@ips; $i++) {
        system("date");
        $cmd = "euca-disassociate-address $ips[$i]";
        $rc = system($cmd);
        if ($rc) {
        print "ERROR: failed - '$cmd'\n";
        }
