...
sub getproperty {
    ...
    if (! $key || $key eq "") {
	doexit(1, "ERROR: no key to get ($key\n");
    }
    
    $cmd = "$runat $remote_pre euca-describe-properties | grep $key $remote_post";
    my ($rc, $crc, $buf) = piperun($cmd, "", "ubero");
    if ($rc || $crc || !$buf) {
        print "Could not get property: $key\n";
        return(1);
    } else {
        $current_artifacts{'systemproperty'} = "$buf";
    }
    return(0);
}
